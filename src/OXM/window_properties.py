...
class oxcWindowProperties:
    ...
    def on_addcustomfield_clicked(self, widget, data=None):
        ...
        if self.builder.get_object("namecustomfields").get_text():
            ...
            ctype = listcombocustomfields.get_value(
                combocustomfields.get_active_iter(), 1
            )
    ...
    def on_acceptwcustomfields_clicked(self, widget, data=None):
        ...
        for i in range(listcustomfields.__len__()):
            ...
            oxm_xml += (
                '<CustomFieldDefinition name="%s" type="%s" defaultValue="" />'
                % (
                    listcustomfields.get_value(iter_ref, 1),
    ...
    def on_acceptdialogoptions_clicked(self, widget, data=None):
        ...
        self.config["gui"]["save_password"] = str(
            self.builder.get_object("checksavepassword").get_active()
        )
    ...
    def on_btmoveup_clicked(self, widget, data=None):
        ...
        """
        rows = (
            self.builder.get_object("treebootorder")
            .get_selection()
    ...
    def prop_visible_func(self, model, iter, user_data=None):
        ...
        if self.selected_type == "host":
            if (
                ...
                and gtk.Buildable.get_name(self.selected_widget)
                == "bthostnetworkproperties"
            ):
                ...
                if not host.count(aka):
                    return False
        elif self.selected_widget and (
            gtk.Buildable.get_name(self.selected_widget) == "btstorageproperties"
            or gtk.Buildable.get_name(self.selected_widget) == "btstgproperties"
        ):
            # same
            ...
            self.selected_widget
            and gtk.Buildable.get_name(self.selected_widget) == "menuitem_server_prop"
        ):
            # same
            if not host.count(aka):
                return False
            ...
            or self.selected_type == "template"
            or self.selected_type == "custom_template"
        ):
            if (
                "HVM_shadow_multiplier"
                in self.xc_servers[self.selected_host].all["vms"][self.selected_ref]
                and "HVM_boot_policy"
                in self.xc_servers[self.selected_host].all["vms"][self.selected_ref]
                and self.xc_servers[self.selected_host].all["vms"][self.selected_ref][
                    "HVM_boot_policy"
                ]
            ):
    ...
    def on_combostgmodeposition_changed(self, widget, data=None):
        ...
        if self.selected_widget:
            ...
            if gtk.Buildable.get_name(self.selected_widget) == "btstgproperties":
                ref = self.xc_servers[self.selected_host].all["VDI"][ref]["VBDs"][
                    path[0] - 9
                ]
                device = self.xc_servers[self.selected_host].all["VBD"][ref][
                    "userdevice"
                ]
            ...
            if gtk.Buildable.get_name(widget) == "combostgmode":
                ...
                if device != active_text and not self.freedevices[vm_ref].count(
                    active_text
                ):
    ...
    def on_treeprop_button_press_event(self, widget, event):
        ...
        if pthinfo is not None:
            ...
            if path[0] < 9 or self.listprop.get_value(iter, 2) == "advancedoptions":
                self.builder.get_object("tabprops").set_current_page(
                    self.listprop.get_value(iter, 3)
                )
            else:
            ...
            if path[0] > 8 and self.listprop.get_value(iter, 2) != "advancedoptions":
                ...
                if gtk.Buildable.get_name(self.selected_widget) == "btstgproperties":
                    ...
                    mode = self.xc_servers[self.selected_host].all["VBD"][ref]["mode"]
                    bootable = self.xc_servers[self.selected_host].all["VBD"][ref][
                        "bootable"
                    ]
                    vm_name = self.xc_servers[self.selected_host].all["vms"][vm_ref][
                        "name_label"
                    ]
                    ...
                    if device[0] != "x":
                        ...
                        self.builder.get_object("combostgposition").set_active(
                            int(device)
                        )
                    else:
                        self.builder.get_object("combostgposition").set_sensitive(False)
                    self.builder.get_object("combostgmode").set_sensitive(
                        type == "Disk"
                    )
    ...
    def on_btvmpropaccept_activate(self, widget, data=None):
        ...
        if (
            ...
            if (
                "automatic" in network["other_config"]
                and network["other_config"]["automatic"] == "true"
            ):
            ...
            for cfield in self.vboxchildtext:
                if "XenCenter.CustomFields." + cfield in other_config:
                    if (
                        self.vboxchildtext[cfield].get_text()
                        != other_config["XenCenter.CustomFields." + cfield]
                    ):
                        change = True
                        other_config["XenCenter.CustomFields." + cfield] = (
                            self.vboxchildtext[cfield].get_text()
            ...
            if change:
                self.xc_servers[self.selected_host].set_network_other_config(
                    self.selected_ref, other_config
                )

        elif self.selected_widget and (
            gtk.Buildable.get_name(self.selected_widget) == "btstgproperties"
            or gtk.Buildable.get_name(self.selected_widget) == "btstorageproperties"
        ):
            if gtk.Buildable.get_name(self.selected_widget) == "btstgproperties":
                liststorage = self.builder.get_object("liststg")
                treestorage = self.builder.get_object("treestg")
            ...
            if self.builder.get_object("txtpropvmname").get_text() != vdi["name_label"]:
                self.xc_servers[self.selected_host].set_vdi_name_label(
                    vdi_ref, self.builder.get_object("txtpropvmname").get_text()
                )
            if (
                tb.get_text(tb.get_start_iter(), tb.get_end_iter())
                != vdi["name_description"]
            ):
                self.xc_servers[self.selected_host].set_vdi_name_description(
                    vdi_ref, tb.get_text(tb.get_start_iter(), tb.get_end_iter())
                )
            if (
                self.builder.get_object("spinvdisize").get_value()
                != float(vdi["virtual_size"]) / 1024 / 1024 / 1024
            ):
                size = (
                    self.builder.get_object("spinvdisize").get_value()
                    * 1024
            ...
            for ref in self.changes:
                if "position" in self.changes[ref]:
                    self.xc_servers[self.selected_host].set_vbd_userdevice(
                        ref, self.changes[ref]["position"]
                    )
                if "mode" in self.changes[ref]:
                    self.xc_servers[self.selected_host].set_vbd_mode(
                        ref, self.changes[ref]["mode"]
                    )
                if "bootable" in self.changes[ref]:
                    self.xc_servers[self.selected_host].set_vbd_bootable(
                        ref, self.changes[ref]["bootable"]
                    )
            ...
            if change:
                self.xc_servers[self.selected_host].set_vdi_other_config(
                    self.selected_ref, other_config
                )
            ...
            if (
                "syslog_destination"
                in vm["logging"]
                == self.builder.get_object("radiologlocal").get_active()
            ):
            ...
            if change:
                self.xc_servers[self.selected_host].set_host_other_config(
                    self.selected_ref, other_config
                )
            ...
            if change:
                self.xc_servers[self.selected_host].set_storage_other_config(
                    self.selected_ref, other_config
                )
            ...
            if change:
                self.xc_servers[self.selected_host].set_pool_other_config(
                    self.selected_ref, other_config
                )
            ...
            if (
                int(self.builder.get_object("spinpropvmmem").get_value())
                != int(vm["memory_dynamic_min"]) / 1024 / 1024
            ):
                self.xc_servers[self.selected_host].set_vm_memory(
                    self.selected_ref,
                    self.builder.get_object("spinpropvmmem").get_value(),
                )
            if int(self.builder.get_object("spinpropvmvcpus").get_value()) != int(
                vm["VCPUs_at_startup"]
            ):
            ...
            if "weight" in vm["VCPUs_params"]:
                if self.builder.get_object("spinpropvmprio").get_value() != float(
            ...
            if (
                "auto_poweron" in vm["other_config"]
                and vm["other_config"]["auto_poweron"] == "true"
            ):
            ...
            if not vm["HVM_boot_policy"]:
                ...
                for i in range(0, 4):
                    ...
                    order += self.builder.get_object("listpropbootorder").get_value(
                        iter, 0
                    )
                    if not self.builder.get_object("listpropbootorder").get_value(
                        iter, 2
                    ):
                        break
            ...
            for vbd_ref in vm["VBDs"]:
                ...
                if vbd["VDI"] != "OpaqueRef:NULL" and vbd["VDI"]:
                    ...
                    if not self.xc_servers[self.selected_host].get_storage(vdi["SR"])[
                        "shared"
                    ]:
                        shared = False
                        break
            if self.builder.get_object("radioautohome").get_active() and (
                vm["affinity"] != "OpaqueRef:NULL"
            ):
                self.xc_servers[self.selected_host].set_vm_affinity(
                    self.selected_ref, "OpaqueRef:NULL"
                )
            ...
            if change:
                self.xc_servers[self.selected_host].set_vm_other_config(
                    self.selected_ref, other_config
                )
            ...
            if "HVM_shadow_multiplier" in vm:
                ...
                if multiplier != str(vm["HVM_shadow_multiplier"]):
                    self.xc_servers[self.selected_host].set_vm_memory_multiplier(
                        self.selected_ref, multiplier
                    )
    ...
    def fill_btstorage_properties(self, widget):
        ...
        if selection.get_selected()[1] is not None:
            ...
            vdi_sr = self.xc_servers[self.selected_host].all["VDI"][vdi_ref]["SR"]
            vdi = self.xc_servers[self.selected_host].all["VDI"][vdi_ref]
            stg_name = self.xc_servers[self.selected_host].all["SR"][vdi_sr][
                "name_label"
            ]
            stg_pbds = self.xc_servers[self.selected_host].all["SR"][vdi_sr]["PBDs"]
            hosts = []
            for stg_pbd in stg_pbds:
                stg_host = self.xc_servers[self.selected_host].all["PBD"][stg_pbd][
                    "host"
                ]
                hosts.append(
                    self.xc_servers[self.selected_host].all["host"][stg_host][
                        "name_label"
                    ]
                )
            ...
            self.builder.get_object("txtpropvmdesc").get_buffer().set_text(
                vdi["name_description"]
            )
            iter = listprop.get_iter((8,))
            subtext = (
                self.convert_bytes(vdi["virtual_size"])
                + ","
                + stg_name
                + " on "
                + ",".join(hosts)
            ...
            listprop.set_value(
                iter, 1, "<b>Size and Location</b>\n   <i>" + subtext + "</i>"
            )
            self.builder.get_object("adjvdisize").set_lower(
                float(vdi["virtual_size"]) / 1024 / 1024 / 1024
            )
            self.builder.get_object("spinvdisize").set_value(
                float(vdi["virtual_size"]) / 1024 / 1024 / 1024
            )
            listvdilocation = self.builder.get_object("listvdilocation")
            pos = self.xc_servers[self.selected_host].fill_vdi_location(
                vdi_sr, listvdilocation
            )
            self.builder.get_object("spinvdisize").set_sensitive(
                vdi["allowed_operations"].count("resize")
            )
            if gtk.Buildable.get_name(widget) == "btstgproperties":
                i = 9
                ...
                for ref in self.xc_servers[self.selected_host].all["VDI"][ref]["VBDs"]:
                    ...
                    listprop.append(
                        [
                            GdkPixbuf.Pixbuf.new_from_file(
                                path.join(utils.module_path(), "images/prop_stgvm.png")
                            ),
                            "<b>NAME</b>",
                            "stgvm",
                            i,
                    ...
                    i += 1
                    self.freedevices[vm_ref] = self.xc_servers[
                        self.selected_host
                    ].get_allowed_vbd_devices(vm_ref)
                    update += parts
    ...
    def fill_host_network_properties(self, widget):
        ...
        if selection.get_selected()[1] is not None:
            ...
            network = self.xc_servers[self.selected_host].all["network"][
                listnetwork.get_value(tree_iter, 8)
            ]

            tree_iter = listprop.get_iter((0,))
            listprop.set_value(
                tree_iter, 1, "<b>General</b>\n   <i>" + network["name_label"] + "</i>"
            )
    ...
    def fill_server_properties(self, widget):
        ...
        if gtk.Buildable.get_name(widget) == "menuitem_server_prop":
            ref = self.treestore.get_value(
                self.treestore.iter_parent(self.selected_iter), 6
            )
    ...
    def fill_vm_properties(self):
        ...
        listprop.set_value(
            ...
            "<b>CPU and Memory</b>\n   <i>"
            + "%s VCPU(s) and %s RAM"
            % (vm["VCPUs_at_startup"], self.convert_bytes(vm["memory_dynamic_max"]))
            + "</i>",
        )

        self.builder.get_object("spinpropvmmem").set_value(
            float(vm["memory_dynamic_min"]) / 1024 / 1024
        )
        self.builder.get_object("spinpropvmvcpus").set_value(
            float(vm["VCPUs_at_startup"])
        )
        if "weight" in vm["VCPUs_params"]:
            self.builder.get_object("spinpropvmprio").set_value(
                float(vm["VCPUs_params"]["weight"])
            )
        ...
        if not vm["HVM_boot_policy"]:
            ...
            listbootorder.append(
                [
                    "",
                    "-------------- VM will not boot from devices below this line ------------",
                    False,
                ]
            )
        ...
        for vbd_ref in vm["VBDs"]:
            ...
            if vbd["VDI"] != "OpaqueRef:NULL" and vbd["VDI"]:
                ...
                if (
                    not vdi
                    or self.xc_servers[self.selected_host].get_storage(vdi["SR"])[
                        "type"
                    ]
                    != "nfs"
                ):
        ...
        listhomeserver = self.builder.get_object("listhomeserver")
        server = self.xc_servers[self.selected_host].fill_listhomeserver(
            listhomeserver, vm["affinity"]
        )
        treehomeserver = self.builder.get_object("treehomeserver")
        ...
        if "HVM_shadow_multiplier" in vm:
            self.builder.get_object("memorymultiplier").set_text(
                str(vm["HVM_shadow_multiplier"])
            )
    ...
    def fill_custom_fields_table(self, add=False):
        ...
        self.vboxchildtext = {}
        if (
            "XenCenter.CustomFields"
            in self.xc_servers[self.selected_host].all["pool"][pool_ref]["gui_config"]
        ):
            for ch in self.builder.get_object("vboxcustomfields").get_children():
                self.builder.get_object("vboxcustomfields").remove(ch)

            dom = xml.dom.minidom.parseString(
                self.xc_servers[self.selected_host].all["pool"][pool_ref]["gui_config"][
                    "XenCenter.CustomFields"
                ]
            )
            for node in dom.getElementsByTagName("CustomFieldDefinition"):
                ...
                if name not in self.vboxchildtext:
                    ...
                    try:
                        vboxframe.modify_bg(gtk.STATE_NORMAL, _rgba_white)
                    ...
                    try:
                        vboxevent.modify_bg(gtk.STATE_NORMAL, _rgba_white)
                    except (TypeError, AttributeError):
                        pass
                    self.builder.get_object("vboxcustomfields").pack_start(
                        vboxevent, False, False, 0
                    )
    ...
    def on_properties_activate(self, widget, data=None):
        ...
        self.builder.get_object("tabprops").set_current_page(0)
        if (
            gtk.Buildable.get_name(widget) == "btstorageproperties"
            or gtk.Buildable.get_name(widget) == "btstgproperties"
        ):
            ...
            gtk.Buildable.get_name(widget) == "menuitem_server_prop"
            or self.selected_type == "host"
        ):
